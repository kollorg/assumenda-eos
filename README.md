...
# @kollorg/assumenda-eos [![NPM version][npm-image]][npm-url] [![NPM downloads](https://img.shields.io/npm/dw/@kollorg/assumenda-eos.svg)](https://www.npmjs.com/package/@kollorg/assumenda-eos) [![Build Status](https://github.com/kollorg/assumenda-eos/actions/workflows/ci.yml/badge.svg)](https://github.com/kollorg/assumenda-eos/actions/workflows/ci.yml)

[npm-image]: https://img.shields.io/npm/v/@kollorg/assumenda-eos.svg?style=flat
[npm-url]: https://npmjs.org/package/@kollorg/assumenda-eos
...
Minimal lightweight simple logging for JavaScript (browsers, node.js or elsewhere). @kollorg/assumenda-eos extends `console.log()` & friends with level-based logging and filtering, with none of console's downsides.

Test it out live in your browser console at https://pimterry.github.io/@kollorg/assumenda-eos/demo/index.html
...
## Downloading @kollorg/assumenda-eos

If you're using NPM, you can just run `npm install @kollorg/assumenda-eos`.

Alternatively, @kollorg/assumenda-eos is also available via [Bower](https://github.com/bower/bower) (`bower install @kollorg/assumenda-eos`), as a [Webjar](http://www.webjars.org/), or an [Atmosphere package](https://atmospherejs.com/spacejamio/@kollorg/assumenda-eos) (for Meteor)

Alternatively if you just want to grab the file yourself, you can download either the current stable [production version][min] or the [development version][max] directly, or reference it remotely on unpkg at [`https://unpkg.com/@kollorg/assumenda-eos/dist/@kollorg/assumenda-eos.min.js`][cdn] (this will redirect to a latest version, use the resulting redirected URL if you want to pin that version).

Finally, if you want to tweak @kollorg/assumenda-eos to your own needs or you immediately need the cutting-edge version, clone this repo and see [Developing & Contributing](#developing--contributing) below for build instructions.

[min]: https://raw.github.com/pimterry/@kollorg/assumenda-eos/master/dist/@kollorg/assumenda-eos.min.js
[max]: https://raw.github.com/pimterry/@kollorg/assumenda-eos/master/dist/@kollorg/assumenda-eos.js
[cdn]: https://unpkg.com/@kollorg/assumenda-eos/dist/@kollorg/assumenda-eos.min.js

## Setting it up

@kollorg/assumenda-eos supports AMD (e.g. RequireJS), CommonJS (e.g. Node.js) and direct usage (e.g. loading globally with a `<script>` tag) loading methods. You should be able to do nearly anything, and then skip to the next section anyway and have it work. Just in case, though, here's some specific examples that definitely do the right thing:

### CommonsJS (e.g. Node)

```javascript
var log = require('@kollorg/assumenda-eos');
...
```javascript
define(['@kollorg/assumenda-eos'], function(log) {
...
```html
<script src="@kollorg/assumenda-eos.min.js"></script>
<script>
log.warn("too easy");
</script>
```
...
```javascript
import * as log from '@kollorg/assumenda-eos';
log.warn("module-tastic");
```

There's no major difference, unless you're using TypeScript & building a @kollorg/assumenda-eos plugin (in that case, see <https://github.com/kollorg/assumenda-eos/issues/149>). In general though, just use whichever suits your environment, and everything should work out fine.

### With noConflict()

If you're using another JavaScript library that exposes a `log` global, you can run into conflicts with @kollorg/assumenda-eos. Similarly to jQuery, you can solve this by putting @kollorg/assumenda-eos into no-conflict mode immediately after it is loaded onto the page. This resets the `log` global to its value before @kollorg/assumenda-eos was loaded (typically `undefined`), and returns the @kollorg/assumenda-eos object, which you can then bind to another name yourself.
...
@kollorg/assumenda-eos includes its own type definitions, assuming you're using a modern module environment (e.g. Node.JS, webpack, etc), you should be able to use the ES6 syntax above, and everything will work immediately. If not, file a bug!
...
* Create a `@kollorg/assumenda-eos.d.ts` file
* Ensure that file is included in your build (e.g. add it to `include` in your tsconfig, pass it on the command line, or use `///<reference path="./@kollorg/assumenda-eos.d.ts" />`)
* In that file, add:
...
The @kollorg/assumenda-eos API is extremely minimal. All methods are available on the root @kollorg/assumenda-eos object, which we suggest you name `log` (this is the default if you import it globally, and is what's set up in the above examples). The API consists of:
...
Be aware that this means that these methods won't always produce exactly the output you expect in every environment; @kollorg/assumenda-eos only guarantees that these methods will never explode on you, and that it will call the most relevant method it can find, with your argument. For example, `log.trace(msg)` in Firefox before version 64 prints the stacktrace by itself, and doesn't include your message (see [#84](https://github.com/kollorg/assumenda-eos/issues/84)).
...
```javascript
// In module-one.js:
var log = require("@kollorg/assumenda-eos").getLogger("module-one");
function doSomethingAmazing() {
  log.debug("Amazing message from module one.");
}

// In module-two.js:
var log = require("@kollorg/assumenda-eos").getLogger("module-two");
...
[@kollorg/assumenda-eos-plugin-prefix](https://github.com/kutuluk/@kollorg/assumenda-eos-plugin-prefix) - plugin for @kollorg/assumenda-eos message prefixing.

[@kollorg/assumenda-eos-plugin-remote](https://github.com/kutuluk/@kollorg/assumenda-eos-plugin-remote) - plugin for sending @kollorg/assumenda-eos messages to a remote log server.

[@kollorg/assumenda-eos-serverSend](https://github.com/artemyarulin/@kollorg/assumenda-eos-serverSend) - Forward your log messages to a remote server.

[@kollorg/assumenda-eos-debug](https://github.com/vectrlabs/@kollorg/assumenda-eos-debug) - Control logging from a DEBUG environmental variable (similar to the classic [Debug](https://github.com/visionmedia/debug) module).
...
Including that would increase the size and complexity of the library, but more importantly would remove stacktrace information. Currently log methods are either disabled, or enabled with directly bound versions of the `console.log` methods (where possible). This means your browser shows the log message as coming from your code at the call to `log.info("message!")` not from within @kollorg/assumenda-eos, since it really calls the bound console method directly, without indirection. The indirection required to dynamically format, further filter, or redirect log messages would stop this.

There's clearly enough enthusiasm for this even at that cost that @kollorg/assumenda-eos now includes a plugin API. To use it, redefine `log.methodFactory(methodName, logLevel, loggerName)` with a function of your own. This will be called for each enabled method each time the level is set (including initially), and should return a function to be used for the given log method `methodName`, at the given _configured_ (not actual) level `logLevel`, for a logger with the given name `loggerName`. If you'd like to retain all the reliability and features of @kollorg/assumenda-eos, we recommended that you wrap the initially provided value of `log.methodFactory`.
...
If you develop and release a plugin, please get in contact! I'd be happy to reference it here for future users. Some consistency is helpful; naming your plugin '@kollorg/assumenda-eos-PLUGINNAME' (e.g. @kollorg/assumenda-eos-newsflash) is preferred, as is giving it the '@kollorg/assumenda-eos-plugin' keyword in your `package.json`.
...
To do a release of @kollorg/assumenda-eos:
...
v0.3.0 - Some bugfixes ([#12](https://github.com/kollorg/assumenda-eos/issues/12), [#14](https://github.com/kollorg/assumenda-eos/issues/14)), cookie-based log level persistence, doc tweaks, support for Bower and JamJS
...
v0.6.0 - Handle logging in Safari private browsing mode ([#33](https://github.com/kollorg/assumenda-eos/issues/33)), fix `TRACE` level persistence bug ([#35](https://github.com/kollorg/assumenda-eos/issues/35)), plus various minor tweaks
...
v1.1.0 - Added support for including @kollorg/assumenda-eos with preprocessing and `.apply()` ([#50](https://github.com/kollorg/assumenda-eos/issues/50)), and fixed QUnit dep version which made tests potentially unstable.
...
v1.4.1 - Reorder UMD ([#92](https://github.com/kollorg/assumenda-eos/issues/92)) to improve bundling tool compatibility

v1.5.0 - Fix `log.debug` ([#111](https://github.com/kollorg/assumenda-eos/issues/111)) after V8 changes deprecating console.debug, check for `window` upfront ([#104](https://github.com/kollorg/assumenda-eos/issues/104)), and add `.log` alias for `.debug` ([#64](https://github.com/kollorg/assumenda-eos/issues/64))

v1.5.1 - Fix bug ([#112](https://github.com/kollorg/assumenda-eos/issues/112)) in level-persistence cookie fallback, which failed if it wasn't the first cookie present

v1.6.0 - Add a name property to loggers and add `log.getLoggers()` ([#114](https://github.com/kollorg/assumenda-eos/issues/114)), and recommend unpkg as CDN instead of CDNJS.
...
## `@kollorg/assumenda-eos` for enterprise

Available as part of the Tidelift Subscription.

The maintainers of `@kollorg/assumenda-eos` and thousands of other packages are working with Tidelift to deliver commercial support and maintenance for the open source dependencies you use to build your applications. Save time, reduce risk, and improve code health, while paying the maintainers of the exact dependencies you use. [Learn more.](https://tidelift.com/subscription/pkg/npm-@kollorg/assumenda-eos?utm_source=npm-@kollorg/assumenda-eos&utm_medium=referral&utm_campaign=enterprise&utm_term=repo)
